class ShoppingCart {
  ...
  createItemInCart(product) {
    ...
    if (itemAlreadyInList) {
      itemAlreadyInList.innerHTML = `<p class="shopping-cart__name">${
        product.name
      }</p>
            <div class="shopping-cart__price-box">
                <p class="shopping-cart__quantity">x${this.getItemsAmount(
                  product.name
                )}</p>
                <p class="shopping-cart__price">${(
                  product.price * this.getItemsAmount(product.name)
                ).toFixed(2)}</p>
            </div>`;
    } else {
    ...
    if (!isInList) {
      ...
      listItem.innerHTML = ` <div class="order__product-box">
        <div class="order__product">
            <div class="order__photo-box"><img src="img/menu-${
              item.name
            }.jpg" alt="${item.name}" class="order__photo"></div>
            <p class="order__name">${item.name}</p>
        </div>
        <div class="order__price-box">
            <p class="order__quantity">x${this.getItemsAmount(item.name)}</p>
            <p class="order__price">${(item.price * item.quantity).toFixed(
              2
            )}zł</p>
  ...
  getTotalPrice() {
    ...
    this.totalPrice =
      ...
      this.getAllIngredientsPrice(this.items);
  ...
  hasIngredients(item) {
    let ingredientsPrice = parseFloat(
      this.getIngredientsPriceFromSingleItem(item)
    );
    if (ingredientsPrice > 0) {
      return `+dodatki: ${
        this.isMoreSameItems(item)
          ? this.getAllIngredientsPrice(sameItems).toFixed(2)
          : this.getIngredientsPriceFromSingleItem(item)
      }zł`;
    } else {
      return "";
  ...
  getIngredientsPriceFromSingleItem(item) {
    let ingredientPrice = item.ingredients.reduce(
      (acc, { price, quantity }) => acc + price * quantity,
      0
    );
  ...
  changeCurrentItem(event) {
    if (
      event.target
        .closest("button")
        .classList.contains("ingredients__arrow--right")
    ) {
      index++;
      ...
      if (index >= sameItems.length) {
        index = 0;
        currentItem = sameItems[index];
        dots[sameItems.length - 1].classList.remove(
          "ingredients__dots--current"
        );
        dots[0].classList.add("ingredients__dots--current");
        this.setInputsQuantity();
        this.updateIngredientsPrice(currentItem);
  ...
  getAllIngredientsPrice(ingredients) {
    ...
    ingredients.forEach(item => {
      ingredientPrice = item.ingredients.reduce(
        (acc, { price, quantity }) => acc + price * quantity,
        0
      );
